...
> **Naming note, established rather than assumed.** There is **no plugin named "Graphify"** in the official Obsidian community registry (verified against the full ~6.3k-entry `community-plugins.json`). The plugin matching that description — local graph-theory metrics with AI-assisted insight — is **`knowledge-graph-analysis`** by *luolanaatud*. Note also that `graph-analysis` by *SkepticMystic*, widely cited in older write-ups, is **no longer in the registry**. Verify plugin identity against the registry before adopting; write-ups age faster than the ecosystem.
