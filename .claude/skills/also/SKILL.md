...
5. **Guard the magnitude, not only the direction.** The direction guard is below; this is the one that was missing. Before integrating, size the addition against the in-flight turn's **Tier**, **Fan-out cap** and **Main-line budget**. If its own plan exceeds any of them, **raise the tier explicitly and say so in one line** — *"this addition takes the turn from T0 to T1; fan-out cap 0 → 2"* — rather than absorbing it silently. A raise is a decision the user can see and stop; a silent absorption is how a late thought becomes the most expensive turn of the session. If the addition is large enough to deserve its own goal state, say that instead of growing this turn.
