...
pack/           ← SINGLE SOURCE OF TRUTH — edit here only
  knowledge/    ← 39 knowledge docs (reasoning spine + vendored foundation)
  commands/     ← 28 skills (SKILL.md + reference/ each)
...
### The 28 skills and their natural order
```
...
`/session-profiler` profiles one or more pack-consuming repos' Claude Code and Copilot CLI sessions from their local telemetry into a findings table and a fixes table (performance, efficiency, adherence, fan-out, cross-harness coordination) — the measured input to `/dream`.
