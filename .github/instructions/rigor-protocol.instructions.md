...
These five carry a piece of work from idea to shipped code. **Three supporting skills** run the same protocol specialized to a different end: **`/collectknowledge`** (before design — Stage 3 gathers and sources the domain's state of the art; the artifact is a knowledge base, not a work product), **`/adddomainexperts`** (Stage 1 establishes the domain from repo evidence, Stage 4 attacks the proposed roster as sprawl; the artifact is a roster delta), and **`/document`** (the subject is the codebase itself; Stage 3 extracts the API reference and diagrams from the source, Stage 4 verifies every diagram and example against the code). All three are documented with the personas they concern in `collaborative-personas.md` §5.
