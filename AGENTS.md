...
The pack source is in `pack/`; both installs — the Claude Code surface (`.claude/`) and the
GitHub Copilot surface (`.github/instructions/`, `.github/prompts/`, `.github/agents/`,
`.github/hooks/`) — and the shared `docs/` are generated from `pack/` by `tools/sync-pack.ps1`.
See `README.md` for the full layout. `CLAUDE.md` is `@AGENTS.md` plus a short Claude Code
addendum, so this file is the single instruction set for every harness (INSTALL 1.1).

When you change anything under `pack/`, re-run `tools/sync-pack.ps1`, then run
**`pwsh tools/verify-bundle.ps1`** *before committing* — it runs the same ten gates CI does,
