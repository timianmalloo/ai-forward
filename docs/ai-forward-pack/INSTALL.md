...
bundle_version: '2026.09.14.3'
revision: 72
counts: { lenses: 23, skills: 27, knowledge_docs: 38, templates: 28, scripts: 27 }
...
changes:
  - {"type": "added", "area": "adapters", "paths": ["adapters/codex/", "adapters/hooks/codex.hooks.json", "adapters/hooks/reread-guard.py", "adapters/hooks/session-start.py", "scripts/pack-apply.py", "scripts/pack-doctor.py", "adapters/managed-blocks/AGENTS.block.md", "commands/addpacktorepo/SKILL.md", "commands/updatepack/SKILL.md"], "deploy": "Run the SOURCE copy of pack/scripts/pack-apply.py apply --target <repo>. Codex skills and references go to .agents/skills; source personas become .codex/agents/<name>.toml; native hooks merge into .codex/hooks.json. Keep config.toml and hook trust under user control. RE-PASTE AGENTS.block.md and preserve !.agents/skills/ in Git ignore rules. Review installed hooks in Codex /hooks.", "summary": "Native Codex adapter: one shared projection used by source sync and consumer install, shared AGENTS.md constitution, host tool/persona mapping, native audit and reread hooks, full-content drift checks and installation-health reporting. No model or permission override."}
---
...
<details>
<summary>Revision 71 — 2026-09-14: previous refresh entries</summary>

The exact previous refresh log is preserved in [revision-71.yaml](history/revision-71.yaml). These are historical entries, not current installation instructions.
...
| Pack artifact | Claude Code | GitHub Copilot | Grok Build |
|---|---|---|---|
| Rules of the Road (the always-on rules) | `CLAUDE.md` (link to it) + `.claude/` | `AGENTS.md` | `AGENTS.md` (native) + `.grok/rules/grok-surface.md` (path map only — not a copy of the block) |
| Knowledge docs (`knowledge/*.md`, plus the existing pack) | `.claude/knowledge/` or repo `docs/` referenced from `CLAUDE.md` | `.github/instructions/*.instructions.md` with `applyTo` globs | **shared** `.claude/knowledge/` — do **not** wrap into `.grok/rules/` (CTX-B) |
| The 27 skills (`commands/<name>/SKILL.md` + `reference/*.md`) | `.claude/skills/<name>/` — the **whole directory**: the contract plus the stage files it reads on demand | `.github/prompts/<name>.prompt.md` (wrapper that carries the same flow); Copilot CLI also reads `.claude/skills/` | `.grok/skills/<name>/` (same SKILL.md). Claude-compat (default on) also discovers `.claude/skills/`; `.grok/skills/` wins on name. Pack `/implement` overrides Grok's bundled implement in a pack-installed repo — intended. |
| Thin command entry points | *(none - Claude Code auto-discovers `.claude/skills/*/SKILL.md` by description)* | `.github/prompts/<name>.prompt.md` | slash `/<name>` from `.grok/skills/` (and `.claude/skills/` when Claude-compat is on) |
| Peer agents (orchestrator, product-strategist, domain-researcher) | `.claude/agents/<name>.md` | `.github/agents/<name>.agent.md` | `.grok/agents/<name>.md` — spawn with `spawn_subagent`, `subagent_type` = persona `name` |
| Adversary agents (the existing 11) | `.claude/agents/<name>.md` | `.github/agents/<name>.agent.md` | `.grok/agents/<name>.md` (`*_agent` suffix stripped, `tools:` stripped) |
| Templates (`templates/*`, 24 incl. the glossary, decision note, threat model, privacy review, design-language + preview) | `docs/ai-forward-pack/templates/` (referenced by skills) | same (shared) | same (shared) |
| Script bundle (`scripts/*` — stdlib-only Python tools plus dependency-free `docs-explorer-core.js`) | `docs/ai-forward-pack/scripts/` (`docs-graph.py` owns graph mechanics + bounded grounding packets; `docs-explorer-core.js` owns deterministic browser state/layout; `audit-log.py`/`prompt-log.py` own history/reuse; `design-lint.py` checks design-language references; Python tools need Python 3.8+) | same (shared) | same (shared) |
| Hooks (`adapters/hooks/*`) §1.4 | `docs/ai-forward-pack/hooks/reread-guard.py` + `session-start.py` + merge `claude-code.settings.hooks.json` into `.claude/settings.json` | the same scripts + `copilot.ai-forward-hooks.json` → `.github/hooks/ai-forward.json` | the same scripts + `grok.ai-forward-hooks.json` → `.grok/hooks/ai-forward.json` (requires folder trust) |
| CI reference workflow (`ci/docs-health.yml`) | `.github/workflows/docs-health.yml` (optional but recommended — gates PRs on graph health) | same (shared) | same (shared) |
| Pack regression suite (`evals/`) | **not deployed** — pack-maintenance tooling; lives wherever you maintain the pack source | — | — |
| Pack docs (`README`, `OVERVIEW`, `research-synthesis`, this file) | `docs/ai-forward-pack/` | same (shared) | same (shared) |
| The Docs Explorer (`templates/docs-explorer.template.html`) | `docs/index.html` (one-time copy, `__PROJECT__` substituted) | same (shared) | same (shared) |
| The UI capability guide (`templates/ui-capability-guide.template.html`) | `docs/ui-guide.html` (one-time verbatim copy) + `docs/ui-guide.md` from `templates/ui-guide-hub.template.md` (substitute `__REVIEW_BY__`); registers itself as a **guide** surface on the next `docs-graph.py derive` | same (shared) | same (shared) |
| Managed blocks (`adapters/managed-blocks/*.block.md`) §1.1 | `CLAUDE.md` = `@AGENTS.md` + the short `CLAUDE.block.md` addendum | paste `AGENTS.block.md` into `AGENTS.md` — the one full block, read by Copilot, Claude (via import), and Grok | Grok loads `AGENTS.md` natively; `.grok/rules/grok-surface.md` is the Grok-only path map |
...
python docs/ai-forward-pack/scripts/coord-core.py doctor      # read the state back
```
...
Codex reads the shared `AGENTS.md` constitution. It discovers repository skills at
`.agents/skills/<name>/SKILL.md` and custom personas at `.codex/agents/<name>.toml`.
The source stays in `pack/`; `adapters/codex/render.py` is the shared projection
used by `sync-pack.ps1` and `pack-apply.py`.

| Canonical source | Codex destination and transform |
|---|---|
| `commands/<name>/` | `.agents/skills/<name>/`, whole directory; SKILL.md includes Codex host guidance |
| Claude/Copilot source personas | `.codex/agents/<name>.toml`, name/description/body and knowledge lens; no tools/model/permission overrides |
| `adapters/codex/surface.md` | `docs/ai-forward-pack/codex-surface.md`, referenced from AGENTS.md and embedded in native skill/persona instructions |
| `adapters/hooks/codex.hooks.json` | Merge into `.codex/hooks.json`; unrelated hooks and `config.toml` remain intact |
| Shared hook scripts | `docs/ai-forward-pack/hooks/`, selected with `--host codex` |
| Shared constitution and knowledge | `AGENTS.md` and `.claude/knowledge/`; no second constitution |

Invoke skills with `$name` or `/skills`. Request delegation to a named persona;
if the host's spawning API has no persona selector, pass that persona's generated
developer instructions explicitly to a separate child agent. Tool vocabulary is
mapped in the Codex surface guide, using actual available signatures.

Hooks require project trust and individual definition review through `/hooks`.
The installer does not enable disabled hooks or grant trust. Windows commands
require Python and PowerShell 7 (`pwsh`); POSIX commands use `python3` and Git.
The audit marker resolves the repository root from nested directories. The Codex
reread guard observes single-file `cat [--] PATH` and
`Get-Content [-Path|-LiteralPath] PATH [-Raw]` commands through `Bash`; ambiguous
compound commands and expansions are ignored. It emits advisory context and never
blocks a tool. Fixture execution is not evidence that a user has trusted hooks.

Keep `!.agents/skills/` after the pack's `.agents/*` ignore rule. Source consistency
checks compare every generated file with its canonical projection, including
reference files, personas and hook definitions. Consumer refresh preserves local
deviations through existing reconciliation; a modified pack hook reports a conflict.

Contracts checked against [Codex skills](https://learn.chatgpt.com/docs/build-skills),
[custom agents](https://learn.chatgpt.com/docs/agent-configuration/subagents),
[hooks](https://learn.chatgpt.com/docs/hooks), and
[AGENTS.md](https://learn.chatgpt.com/docs/agent-configuration/agents-md).
Local CLI inspected: 0.154.0. Hook trust remains a runtime/user property.
...
The layers coexist without conflict: `CLAUDE.md`/`.claude/` for Claude Code, `AGENTS.md`/`.github/` for Copilot, `.grok/` for Grok Build. Keep the **knowledge and templates as the single source of truth** in `docs/` (and the shared `.claude/knowledge/` copies), and let each host's skills/prompts reference the same files, so the reasoning protocol, personas, and artifacts stay identical no matter which assistant a developer drives. Update the protocol once; every host inherits it. Grok's `.grok/rules/grok-surface.md` is a path map, not a second constitution — `AGENTS.md` is loaded natively.
