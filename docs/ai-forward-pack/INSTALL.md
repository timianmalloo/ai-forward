...
changes:
  - { type: added, area: skills, paths: ['commands/visualize/SKILL.md', 'adapters/copilot/prompts/visualize.prompt.md', 'evals/cases/visualize-01.json'], deploy: 'copy the skill to .claude/skills/visualize/SKILL.md and the prompt to .github/prompts/visualize.prompt.md; the eval stays with the pack source', summary: 'THE MISSING MECHANISM. Revision 25 shipped ui-visual-assets.md - the doctrine governing generated imagery - but a repo adopting the pack got the rules and NO WAY TO CALL ANYTHING. Doctrine without a mechanism generates nothing. Worse, CD3 deliberately declined Impeccable skill, which is exactly where its visual-world generation lives, so the pack removed a capability and did not replace it. /visualize is the replacement: four modes (board - a direction board that makes a brief concrete, the default when unclear; asset - imagery for named slots; persona - a consistent fictional cast for the review harness; motion - a clip with poster, reduced-motion fallback and a motion-inventory entry). It is runnable STANDALONE against a site you already have, which is the actual use case, and /ui-design hands off to it at Stage 1 and Stage 5. It narrows beautify before generating, because that word means three different jobs and only one of them is this skill. Most of the value sits in Stage 4, the cull.' }
  - { type: added, area: scripts, paths: ['scripts/visual-assets-setup.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'Stdlib backend wiring: --backends prints the capability matrix, --check reports which backend this environment can actually reach and exactly what is missing, --init scaffolds docs/assets/, the assets: manifest in DESIGN.md and the .gitignore hygiene (incl. _scratch/ for candidate boards). Writes NO credential and generates nothing. Two deliberate subtleties: an MCP-reached backend absent from os.environ is reported UNKNOWN rather than unconfigured, because its credentials live in the agent host and saying otherwise would be a confident wrong answer; and unknown does not fail the exit code, because an unknown is a question, not a failure. Also a shallow tripwire for a committed generation credential (exit 2).' }
  - { type: changed, area: knowledge, paths: ['knowledge/ui-visual-assets.md'], deploy: 'copy to .claude/knowledge/ and re-wrap into .github/instructions/; range is now VA1-VA22', summary: 'Added section 6 - the backend layer. VA19 is the finding that motivated it and it is Verified against the provider terms: A CONSUMER AI SUBSCRIPTION IS NOT API ACCESS. A Google AI Pro/Ultra plan grants the Gemini app, Flow and Whisk and grants NO programmatic API access; the API is a separate billing relationship (an AI Studio key on a Cloud project with active billing) and image and video generation are NOT on its free tier. So I have a Google subscription, so my agent can generate is false, and a workflow built on it fails at the first call. Generalised: entitlement is a contract - check which account holds it, whether it exposes an API rather than only a web app, and whether the capability is on the tier being paid for. VA20 is the capability contract a backend must satisfy (text-to-image, image-to-image, image/text-to-video, optional character and preset references, provenance, immediate retrieval). VA21 is the two adapters with verified specifics - Higgsfield over MCP with 121 cinematography motion presets and character references; Google over the google-genai SDK with gemini-3.1-flash-image / gemini-3-pro-image / veo-3.1-generate-preview / gemini-omni-flash-preview, invisible SynthID watermarking, NO official MCP server, and Imagen deprecated. VA22 wires it through the setup script with no committed credential.' }
  - { type: changed, area: templates, paths: ['templates/ui-capability-guide.template.html'], deploy: 'copy to docs/ai-forward-pack/templates/ and re-instantiate docs/ui-guide.html', summary: 'The marketing-surface card now hands off to /visualize; a generation command block and the consumer-subscription-is-not-API-access warning were added to the Commands section.' }
