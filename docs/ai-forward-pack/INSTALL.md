...
bundle_version: '2026.06.30.1'
revision: 14
released: '2026-06-30'
counts: { lenses: 23, skills: 16, knowledge_docs: 24, templates: 17, scripts: 6 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/solution-selection-ladder.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/solution-selection-ladder.instructions.md (applyTo "**")', summary: 'New reasoning-spine doc: the Solution-Selection Ladder — the 7-rung smallest-correct forcing function (YAGNI → reuse-in-codebase → stdlib → native → installed dep → one line → minimum), the inline simplify: marker for bounded shortcuts (ceiling + upgrade trigger), the debt ledger + rot-flag (mapped onto V13/V17), tier-gated ceremony (T0 code-first / T1-T2 full), and the Simplifier delete-list output format. Adapted from the ponytail lazy-senior-dev skill (MIT) and hardened to compose with the Simplifier persona and the pack correctness floors. Knowledge docs 23→24.' }
  - { type: changed, area: skill, paths: ['commands/design/SKILL.md', 'commands/implement/SKILL.md', 'commands/migrate/SKILL.md'], deploy: 'RE-COPY these skills to .claude/skills/ and re-wrap their Copilot prompts in .github/prompts/', summary: 'Wired the ladder in as a forcing function: /design climbs it before naming a pattern or dependency (Stage 2 + DoD); /implement climbs it per task, marks bounded shortcuts with simplify:, runs the Simplifier delete-list at the gate, and tier-gates ceremony (T0 code-first + one runnable check); /migrate adds delete-dont-migrate (rung 1) and dependency-removal-not-swap (rung 5) to scope, plus ceiling+trigger markers on the intentional-difference catalog.' }
  - { type: changed, area: agent, paths: ['adapters/copilot/agents/the-simplifier_agent.md'], deploy: 'copy to .claude/agents/ unchanged (Copilot agent format has no tools: line)', summary: 'The Simplifier gained its build-review output procedure — a tagged delete-list (delete/stdlib/native/yagni/shrink, ending net: -N lines possible) as the adversarial mirror of the ladder; its reading list now cites solution-selection-ladder.md.' }
  - { type: changed, area: managed-block, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE both managed blocks wholesale between their markers', summary: 'Added a Smallest correct (build discipline) bullet pointing at solution-selection-ladder — the ladder summary, the simplify: marker, tier-gated ceremony, and the Simplifier as its adversarial mirror.' }
---
