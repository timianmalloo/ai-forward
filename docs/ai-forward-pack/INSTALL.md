...
changes:
  - { type: changed, area: knowledge, paths: ['knowledge/ui-craft-detection.md'], deploy: 'copy to .claude/knowledge/ and re-wrap into .github/instructions/', summary: 'ADDED CD20 - a client-rendered surface is invisible to static scanning, and that silence reads as a pass. Found by running the pack own control over the pack own public explainer: 68,522 body bytes, of which 216 survive stripping the script tags. The detector reported 3 Minor findings because it scanned 216 bytes of shell, not the 68 KB of runtime-generated surface - the report was honest about what it scanned and useless about what shipped. CD20 gives the measurement (strip script, count what remains) and the escalation (detect <url> against the rendered DOM), plus two operational facts established by attempting it: URL scanning REQUIRES puppeteer, which is not in the base install and is its own dependency decision; and a surface scannable only while running has a weaker control than one scannable from source - which is a further argument for the pack own dependency-free HTML artifacts. Range cited as CD1-CD20 across skills, managed blocks and the guide.' }
  - { type: changed, area: knowledge, paths: ['knowledge/continuous-improvement.md'], deploy: 'copy to .claude/knowledge/ and re-wrap into .github/instructions/', summary: 'Seed register gains E2E-H (static control scanned the shell, not the surface) - the sibling of E2E-E (a gate green result is evidence the gate passed, not its contents) and E2E-F (an exit code is not a result). Control: measure the shell fraction before trusting a clean run; scan the rendered DOM where the surface is runtime-built.' }
  - { type: added, area: docs, paths: ['docs/reviews/ui-pack-explainer.md'], deploy: 'repo-local review artifact; not deployed', summary: 'A /ui-design REVIEW of web/ai-forward-pack-explainer.html, run to answer "can Higgsfield beautify the website?" with measurement rather than taste - and the measurement changed the answer. Verdict BLOCK on four blockers: the public surface renders BLANK without three unpkg CDN scripts (React/ReactDOM/htm) carrying ZERO integrity hashes, and has zero :focus-visible rules, zero aria attributes and zero prefers-reduced-motion against 2 animation declarations. Plus 166 distinct hex colours against 20 custom properties, no DESIGN.md governing web/ at all, and 15 font-size literals including sub-11px and half-pixel values. Imagery is not a finding anywhere in the review; the highest improvement-to-effort change is inlining the runtime, for which revision 17 already set the in-repo precedent when the audit viewer dropped its CDN runtime.' }
