...
bundle_version: '2026.08.02.2'
revision: 19
released: ''
counts: { lenses: 23, skills: 18, knowledge_docs: 29, templates: 22, scripts: 10 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/obsidian-lens.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/obsidian-lens.instructions.md (applyTo "**")', summary: 'NEW STANDARD — the Obsidian lens (OB1-OB14). docs/ is already a valid Obsidian vault (V2 frontmatter), so the lens costs nothing to adopt and must never become a dependency. OB1-OB3 hold the boundary: Obsidian is a reader, frontmatter is the record, no query is load-bearing in a canonical artifact, and any relied-upon insight must also be obtainable dependency-free. OB4 commits the vault CONFIG (app/appearance/core-plugins/community-plugins/graph.json + each plugin manifest.json for version provenance) and ignores the per-user STATE (workspace, caches, plugin main.js/styles.css/data.json) — refining project-memory M9, which said ignore .obsidian/ wholesale. OB7 colours the graph by the pack own artifact types with status overlays last; OB8 names what Obsidian untyped graph cannot show and covers it with Breadcrumbs/Dataview/--analyze; OB9-OB11 make each plugin earn its seat, install code only with consent, and treat AI plugin features as a data-egress decision for the Privacy lens; OB13-OB14 read --analyze as a prompt not a gate and watch the dominant hub.' }
  - { type: added, area: scripts, paths: ['scripts/obsidian-setup.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'NEW DEPLOYABLE SCRIPT — stdlib-only, idempotent, --dry-run on every mutating mode. --check reports app/config/index/lens state and prints why each recommended plugin earns its seat; --install-app installs Obsidian per OS (winget/brew/flatpak); --init writes the committed vault config, the graph colour groups keyed to artifact type, two non-canonical lens notes with V2 frontmatter, and the .gitignore commit/ignore split; --fetch-plugins is an explicit opt-in that downloads pinned plugin releases; --analyze computes degree, EXACT betweenness centrality (Brandes), connected components, orphans, leaves, ownership distribution and structural gaps straight from docs-index.js with no Obsidian and no plugin required — so the insight is reproducible, reviewable and CI-available rather than locked behind a GUI.' }
  - { type: changed, area: knowledge, paths: ['knowledge/project-memory-and-obsidian.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instruction', summary: 'M9 refined from "add .obsidian/ and git-ignore it wholesale" to the config/state split, with the rationale recorded: ignoring the config was right while the lens was a private convenience and wrong once it is shared, because every contributor then rebuilds the same colour groups by hand. M8 (reader never writer) is unchanged and is restated as obsidian-lens OB1. Checklist and references updated to hand off to the new standard.' }
  - { type: changed, area: managed-blocks, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE the managed blocks wholesale between their markers in CLAUDE.md / AGENTS.md', summary: 'RE-PASTE REQUIRED. Adds the Obsidian lens bullet — optional, reader-only, config committed and state ignored, with the dependency-free --analyze insight named so the capability is discoverable without implying a dependency.' }
  - { type: changed, area: install-hygiene, paths: ['adapters/INSTALL.md'], deploy: 'add the Obsidian per-user ignore rules to the consuming repository .gitignore (obsidian-setup.py --init does this automatically)', summary: 'The vault config is committed; workspace.json, workspace-mobile.json, .obsidian/cache/ and plugins/*/{main.js,styles.css,data.json} are per-user or third-party code and must stay local.' }
---
