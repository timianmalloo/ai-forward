...
bundle_version: '2026.08.02.1'
revision: 18
released: ''
counts: { lenses: 23, skills: 18, knowledge_docs: 28, templates: 22, scripts: 9 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/domain-and-data-modelling.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/domain-and-data-modelling.instructions.md (applyTo "**")', summary: 'NEW STANDARD — the data model is the highest-priority decision. DM1-DM18: derive the conceptual model in DDD terms (bounded context, ubiquitous language, entities vs value objects, aggregates bounded by an invariant per Vernon''s four rules) before any surface/endpoint/table; then the durable representation — core entities as dimensions, change-over-time as append-only facts, so history and the audit trail ARE the data rather than a shadow schema and new measures evolve without rewriting history. Declare the grain before the columns, classify additivity (additive/semi-additive/non-additive), decide the history rule per attribute (Type-2 vs a recorded decision to discard history), derive-don''t-store, enforce AND test the append-only/interval invariants, snowflake only on domain justification, expand-migrate-contract with a backfill that never guesses. Keeps the ODS-vs-star layering honest. Owned by the Data & Persistence Architect (hard veto).' }
  - { type: added, area: knowledge, paths: ['knowledge/end-to-end-integrity.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/end-to-end-integrity.instructions.md (applyTo "**")', summary: 'NEW STANDARD — the standing method. E1-E18: rigor and adversarial review are unconditional (the absence of "use the Rigor Protocol" is not permission to skip it); route by risk shape not request shape; ground in the whole intent end-to-end and name what a decision constrains; write the change-surface list first (store → model → service → projection/wire → client type → UI → compute reader); every field needs a traced writer AND compute reader; prove the rendered surface through the real composition root and prove consistency ACROSS surfaces; a gate''s green result is evidence the gate passed not that its contents passed; an exit code is not a result — read the state; never assert the shape of our own code from memory; delegated inventory is evidence to verify; close with Completed/Remaining/Next.' }
  - { type: added, area: knowledge, paths: ['knowledge/continuous-improvement.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/continuous-improvement.instructions.md (applyTo "**")', summary: 'NEW STANDARD — continuous improvement as a primary directive. CI1-CI12: every defect, falsified assumption and human correction is captured as a CLASS not an instance, via class → sweep → derive → prevent, into a committed defect-class register (docs/lessons/defect-classes.md) read at grounding. A lesson recorded as prose is a memoir — it must climb the control ladder (make it impossible > automated control > always-loaded instruction > knowledge doc > register-only) and be observed failing on the un-fixed code. Ships a seed register of 20 defect classes observed in production across two independent pack repos (DM-A..F, E2E-A..G, RIG-A..C, REC-A, OPS-A, UX-A..B).' }
  - { type: added, area: knowledge, paths: ['knowledge/ui-design-craft.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/ui-design-craft.instructions.md (applyTo "**")', summary: 'NEW STANDARD — UI direction, prototyping and critique. DX1-DX25: generic is the default output and only process defeats it — separate creative direction from implementation, build the design system before the screens, anchor to named references (adapt never clone), and self-check against the generic-AI-look tells table. The direction brief (user + emotional state, JTBD, Archetype Signature, three adjectives AND their opposites, anti-goals). The fidelity ladder; the self-contained dependency-free mockup rendering the hard states; the REVIEW HARNESS (persona · viewport · state · theme/density · capability flags · reduced motion) with in-artifact contrast/target audit. Visual craft (scale contrast, spacing-as-grouping, restraint, optical alignment, one defended focal point). Motion inventory. Copy as design. The critique rubric — 18 dimensions (Nielsen 10 + pack floors + craft), severity 0-4 mapped to Blocker/Major/Minor/Nit, measure-before-diagnose, structure-before-surface, ranked plan with the highest-leverage change named.' }
  - { type: added, area: skills, paths: ['commands/ui-design/SKILL.md', 'adapters/copilot/prompts/ui-design.prompt.md', '../evals/cases/ui-design-01.json'], deploy: 'copy SKILL.md to .claude/skills/ui-design/SKILL.md and the prompt to .github/prompts/ui-design.prompt.md', summary: 'NEW SKILL /ui-design (the 18th) — create, review or elevate a user-facing surface to a professional standard. Three modes (create · review · elevate; default to review before rebuilding an existing surface). Produces a self-contained mockup + review harness under docs/mockups/, the DESIGN.md design language, and a rubric review at docs/reviews/. Distinct from /design, which owns the component contracts beneath the surface. Led by UX & Accessibility (a11y hard veto, never self-cleared) with UX Researcher/IA holding the UX-specification veto.' }
  - { type: added, area: templates, paths: ['templates/mockup-harness.template.html', 'templates/ui-review.template.md', 'templates/defect-classes.template.md'], deploy: 'copy to docs/ai-forward-pack/templates/', summary: 'Mockup harness — a self-contained, dependency-free, no-CDN HTML mockup with a review bar (persona/viewport/state/theme/density/AI-flag/reduced-motion), rendered hard states (empty/loading/error/partial/overflow), and a live in-artifact WCAG contrast + target-size audit computed from the rendered tokens. UI review — the rubric artifact: measurements, 18-dimension scorecard, generic-tells self-check, Simplifier delete-list, ranked plan. Defect classes — the continuous-improvement register, pre-seeded with the 20 inherited classes.' }
  - { type: changed, area: skills, paths: ['commands/specify/SKILL.md', 'commands/define-architecture/SKILL.md', 'commands/design/SKILL.md', 'commands/implement/SKILL.md', 'commands/investigate/SKILL.md', 'adapters/copilot/prompts/specify.prompt.md', 'adapters/copilot/prompts/define-architecture.prompt.md', 'adapters/copilot/prompts/design.prompt.md', 'adapters/copilot/prompts/implement.prompt.md', 'adapters/copilot/prompts/investigate.prompt.md'], deploy: 'copy to .claude/skills/<name>/SKILL.md and .github/prompts/<name>.prompt.md', summary: 'Data-model primacy, end-to-end integrity and continuous improvement wired into the workflow as gates, not prose. /specify: Part A carries the conceptual domain model (before the UX/UI layers) and the Data & Persistence Architect joins the cast. /define-architecture: the domain model and durable representation are first-class ADR decisions with the read-path trade accepted explicitly. /design: the data model is settled first (aggregates+invariants, grain, additivity, history rule, derive-don''t-store, tested append-only invariants, migration) and the change-surface list is enumerated. /implement: the surface list is ticked off, every field gets a traced writer and compute reader, the rendered surface and cross-surface consistency are proven, and defect classes are registered. /investigate: simplify: markers are harvested and the defect CLASS is registered with a control observed failing.' }
  - { type: changed, area: personas, paths: ['adapters/claude-code/agents/data-persistence-architect.md', 'adapters/claude-code/agents/ux-accessibility.md'], deploy: 'copy to .claude/agents/ (and .github/agents/ for Copilot, stripping any tools: line)', summary: 'Data & Persistence Architect now owns domain-and-data-modelling.md end to end — the conceptual DDD model, the durable representation ADR, grain/additivity/history per attribute, derive-don''t-store, tested invariants, write/read completeness — and owns defect classes DM-A..DM-F. UX & Accessibility now leads /ui-design and works to ui-design-craft.md (direction brief, system-before-screens, mockup + harness, motion inventory, generic-tells self-check).' }
  - { type: changed, area: managed-blocks, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE the managed blocks wholesale between their markers in CLAUDE.md / AGENTS.md', summary: 'RE-PASTE REQUIRED. Adds three always-on directives — the standing method (rigor is unconditional; never decide in a silo; the change-surface list; prove the rendered surface; read the state), data-model primacy (DDD + dimensions/append-only facts + grain/additivity/history/derive-don''t-store), and continuous improvement (capture the class, build the control, prose is a memoir) — plus the /ui-design skill and the UI craft standard. Also FIXES a pre-existing defect: the "UI:" bullet was truncated mid-sentence with its continuation orphaned onto the archetype bullet. Skill count 17→18.' }
  - { type: added, area: docs, paths: ['../docs/knowledge/domain-and-data-modelling/'], deploy: 'source-only evidence base for the new standard; a consuming repo builds its own docs/knowledge/', summary: 'The /collectknowledge evidence base backing the standard: 8 files (index, state-of-the-art, comparables, references, data-and-constants, glossary, open-questions, sources) covering DDD aggregate rules, the three model levels, Kimball grain/additivity/SCD, the ODS-vs-star correction, snowflaking, and the temporal-table-vs-append-only-facts trade-off — every load-bearing claim confidence-labelled, with the disconfirming case against the standard''s central stance written out.' }
---
