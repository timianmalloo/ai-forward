...
changes:
  - { type: changed, area: scripts, paths: ['scripts/obsidian-setup.py'], deploy: 'copy to docs/ai-forward-pack/scripts/; in a repo that already ran --init, also repoint the link in its docs/lenses/graph-health.md', summary: 'DEFECT FIX. The generated graph-health lens hardcoded a link to `docs-index`, which is not a graph node - docs/docs-index.js is the derived data file, not an artifact with frontmatter - so `docs-graph.py validate` reported a dangling link in EVERY repo that ran --init, failing the docs-health gate the pack itself recommends. Verified in two consuming repos and in this one. Now points at `lens-graph-structure`, matching the sibling lenses.' }
---
