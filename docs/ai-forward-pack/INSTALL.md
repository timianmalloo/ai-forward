...
changes:
  - { type: changed, area: adapters, paths: ['adapters/INSTALL.md', 'tools/sync-pack.ps1'], deploy: 'copy INSTALL to docs/ai-forward-pack/; re-run sync', summary: 'FR-032 RESOLVED - the Copilot surface shipped 11 of 23 personas for twelve revisions, so adopters were missing every peer AND four hard-veto lenses. sync now deploys both agent sources to .github/agents, stripping the tools: line at the boundary. A SECOND defect surfaced while gating it: the deploy copied the SOURCE filename (<name>_agent.md) verbatim instead of renaming to Copilot documented <name>.agent.md. All 23 loaded anyway, which nearly led to correcting the map to match the accident - registered as RIG-E.' }
  - { type: changed, area: scripts, paths: ['tools/check-consistency.py'], deploy: 'repo-local tooling; not deployed', summary: 'Two new gates, both proved red-first. check_deployed_agent_parity() counts the DEPLOYED agent directories rather than the pack source - the precise blind spot that let FR-032 survive, since every existing check read pack/ and printed a correct 23. check_directive_ranges() parses the highest directive actually defined in each standard and fails any citation that outruns it; a shorter sub-range stays legitimate.' }
  - { type: changed, area: knowledge, paths: ['knowledge/specification-standards.md', 'knowledge/ui-archetype-grammar.md', 'commands/specify/SKILL.md', 'commands/design/SKILL.md', 'commands/ui-design/SKILL.md', 'knowledge/persona-audit.md', 'knowledge/ui-design-craft.md', 'knowledge/ui-archetype-catalog.md'], deploy: 'copy knowledge to .claude/knowledge/ and re-wrap into .github/instructions/; copy skills', summary: 'FR-035 RESOLVED - the specification standard was cited with a range eight directives longer than it defines, and the archetype grammar with two more than it defines, across 17 source files including three skill Authority lines. Every one pointed adopters at directives that do not exist. Corrected and gated.' }
  - { type: changed, area: knowledge, paths: ['knowledge/continuous-improvement.md'], deploy: 'copy to .claude/knowledge/ and re-wrap into .github/instructions/', summary: 'Seed register gains RIG-E - it works cited as conformance. Observing an artifact function proves it is tolerated, not that it is specified; the tolerant path silently drops the guarantee the standard carried.' }
  - { type: changed, area: scripts, paths: ['scripts/scrub.py', 'scripts/visual-assets-setup.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'FR-042 - scrub allowlists @users.noreply.github.com via negative lookahead, so the GitHub noreply commit identity stops being reported as PII; verified it still flags a real address in the same probe. FR-038 - the committed .mcp.json.example now carries a portable placeholder while the git-ignored .mcp.json keeps the machine-resolved path.' }
  - { type: changed, area: docs, paths: ['docs/backlog/forensic-review.md', 'docs/architecture.md', 'docs/index.md', '.github/workflows/pack-consistency.yml', '.github/CODEOWNERS', 'package.json'], deploy: 'repo-local; not deployed', summary: 'FR-033/034 - CI now gates source-install drift (sync then git diff --exit-code), runs pytest, and runs docs-graph validate; path filters broadened. FR-037 - the proposal was overstated: the model-orchestration references were already explicitly historical, so the real scope was one present-tense claim in index.md plus an architecture.md status row still asserting the very mismatch FR-032 had just fixed. FR-040/041 - stray @mallalieut handle corrected, CODEOWNERS added, npm test defined. Backlog marks nine of twelve resolved.' }
