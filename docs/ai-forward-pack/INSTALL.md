...
changes:
  - { type: changed, area: script, paths: ['scripts/prompt-log.py'], deploy: 'copy to docs/ai-forward-pack/scripts/ (replaces the revision-11 version)', summary: 'UNIFIED the prompt-log with the audit log — one store of prompts, not two. prompt-log.py is now a reuse lens over the committed audit log (docs/audit/audit-log.jsonl): its browse/search/list/show/get read the audit log (adapting each entry prompt→reusable text), and `add` writes a kind:prompt entry THROUGH audit-log.py (AL0.1, the single writer) instead of a separate .aiforward/prompts.jsonl. So every recorded prompt (skill runs, scripts, logged prompts) is reusable via /prompts + /searchprompts AND visible in /auditlog. A legacy .aiforward store is read only via an explicit --store. (Fixed a pre-existing flaky self-test: an all-digit id-prefix was mis-read as an index.)' }
  - { type: changed, area: knowledge, paths: ['knowledge/audit-and-change-log.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instruction', summary: 'Added directive AL13 — the prompt-reuse lens (/prompts, /searchprompts) is a second lens over the ONE store, not a second store; documents the kind:prompt entry written via audit-log.py and the legacy-store --store escape hatch.' }
  - { type: changed, area: skill, paths: ['commands/prompts/SKILL.md', 'commands/searchprompts/SKILL.md', 'adapters/copilot/prompts/prompts.prompt.md', 'adapters/copilot/prompts/searchprompts.prompt.md', 'commands/auditlog/SKILL.md', 'adapters/copilot/prompts/auditlog.prompt.md'], deploy: 'RE-COPY these skills + prompts to .claude/skills/ and .github/prompts/', summary: 'The prompt-reuse skills now describe the unified audit-log store (was .aiforward/prompts.jsonl); /auditlog cross-references /prompts + /searchprompts as the fast reuse lens over the same store.' }
  - { type: changed, area: managed-block, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE both managed blocks wholesale between their markers', summary: 'Prompt-log bullet now says: a reuse lens over the SAME committed audit log (not a second store); `prompt-log.py add` writes a kind:prompt audit entry via audit-log.py.' }
---
