...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/no-guessing-protocol.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/no-guessing-protocol.instructions.md (applyTo "**")', summary: 'NEW STANDARD — NG1-NG11, the mechanism behind the pack''s oldest prohibition. The BoK already forbids guessing (D2, Part VIII) and E15 already forbids asserting own-code shape from memory, yet agents keep saying "I guessed" or "I assumed" AFTER a bug — because a guess and a fact feel identical from the inside, so a prohibition alone is unenforceable. This doc supplies the mechanism instead: NG1 gives exactly THREE permitted moves when you do not know (check it / mark it / ask) and states plainly that proceeding on an unmarked belief is not a fourth option; NG2 is the sharp one — an assumption not written down before the work is not an assumption, it is a guess, which makes "I assumed X" unavailable as a post-hoc explanation; NG3 lists the linguistic and structural tells (should be / presumably / I believe / it follows the pattern; any API, version, path, count or boundary behaviour asserted unopened); NG4 adds the inline `assume:` marker (sibling of `simplify:`) carrying belief + confirmation route + consequence; NG5 is the forcing function — if checking is cheaper than marking you may not mark, so the disciplined path is also the lazy path; NG6 states that Verified means observed, never likely; NG7 forbids laundering a guess through a citation, a tool, a sub-agent or an INFERRED edge; NG9 makes a guess-caused bug a defect CLASS with a control; NG10 requires the closing summary to split Verified/Inferred/Flagged. Section 2 is the moment-of-writing intervention: "if this is wrong, how would I find out, and when?" — if the answer is "in production", check it now.' }
  - { type: changed, area: knowledge, paths: ['knowledge/end-to-end-integrity.md', 'knowledge/continuous-improvement.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instructions', summary: 'E15 now names itself the highest-frequency special case of the No-Guessing Protocol and points at the general mechanism. CI9 extends the investigation marker harvest from `simplify:` to `simplify:` AND `assume:` — an assumption that has come true is the bug, already written down and unread.' }
  - { type: changed, area: skills, paths: ['commands/investigate/SKILL.md'], deploy: 'copy to .claude/skills/investigate/SKILL.md and .github/prompts/investigate.prompt.md', summary: '/investigate now harvests `assume:` markers alongside `simplify:` ones, and states the NG9 class rule: when the root cause is an unmarked belief, the class is not "got X wrong" but "asserted a contract in this domain without checking".' }
  - { type: changed, area: managed-blocks, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE the managed blocks wholesale between their markers in CLAUDE.md / AGENTS.md', summary: 'RE-PASTE REQUIRED. The No-Guessing rule is now the FIRST bullet, above the reasoning spine — three moves, the pre-registration rule, the tells, Verified-means-observed, and the moment-of-writing question. Also adds the Graphify code-knowledge-graph bullet from revision 20.' }
---
