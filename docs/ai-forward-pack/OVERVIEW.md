...
├─ knowledge/   36 docs (+FOUNDATION manifest) — 29 reasoning + 7 vendored Agent-Knowledge-Pack foundation (BoK, Rules of the Road, Persona Catalog, LOA, Governance, Testing Strategy, C# Style)
├─ commands/    (the 22 skills, one SKILL.md each)
├─ templates/   (the 26 artifact templates)
...
There are **22 skills** — six that carry a piece of work from idea to shipped code (`/specify`, `/define-architecture`, `/design`, `/ui-design`, `/implement`, `/investigate`), seven that support them (knowledge collection, persona tailoring, execution-graph planning, documentation, brownfield **adoption**, whole-repo **forensic review**, and characterization-first **migration**), three **pack-lifecycle** skills that manage the pack installation itself (**/addpacktorepo**, **/updatepack**, and **/extendaibundle**), one **utility** skill (**/auditlog**), and two **prompt-log utilities** (**/prompts** and **/searchprompts**). The twelve workflow/support skills form the engineering surface below; lifecycle and utility skills sit outside it.
