...
- **`[Resolved 2026-06-22]` Knowledge graph populated; vendored foundation docs deliberately excluded (decision).** The graph is no longer empty — it holds the project's own dogfood artifacts (`architecture`, the four `design-*` nodes, the six `kb-pack-evolution*` knowledge nodes, the glossary, threat-model, privacy-review, and the `project-memory` ledger). The **22 vendored foundation/standards docs** (`rigor-protocol`, `testing-strategy`, `layered-optimized-architecture`, …) deliberately **stay out** of the dogfood graph: they deploy as *always-applied reference instructions* (`.github/instructions/`, `.claude/knowledge/`), not as navigable `docs/` design artifacts. Adding V2 frontmatter to them would either pollute every always-applied Copilot instruction with graph metadata (the sync wrap prepends `applyTo`, yielding a double-frontmatter block the model reads as body noise) or require deploying duplicate copies under `docs/` — net complexity the Simplifier rejects for no navigational gain (reference material, not project design knowledge). The project's *own* knowledge is in the graph; the vendored constitution is not. `[Decision recorded per BoK Part IX / V17; graph validate = 16 artifacts, 0 orphans]`
...
| **Remaining** | none blocking. (Cosmetic-only: a live *visual* browser pass of the explainer before public share — the parse/structure risk is already retired.) |
| **Best next action** | none outstanding — repo gates green (`check-consistency.py` clean; graph valid + fresh; `BUNDLE CONSISTENT`). |
