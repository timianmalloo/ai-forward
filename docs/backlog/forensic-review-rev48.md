...
- **Owner:** @timianmalloo · **Next skill:** `/design-slice` for the class fix (it changes a contract between three tools), then `/implement` · **Status:** **FULLY RESOLVED.** Instance at `7bc0cf2`; **detection** at `48e0601` (both derived artifacts drift-gated at gate 1, previously only the portal); **source fix at `9eed844`** — `sync-pack.ps1` derives before building the dependents, which became safe once FR-068 made `docs-index.js` byte-stable. Gate 2 exits 0 on two consecutive syncs. Both halves now hold: the trap is closed at source *and* a hand-run that skips sync is still caught by detection.
...
- **Dependencies:** none · **Owner:** @timianmalloo · **Next skill:** `/implement`
...
- **Status:** **RESOLVED at `a9a3a1c`** — by the *first* option (an in-workflow gate), not the required-check option; see the split below.
...
- **Dependencies:** cleanest after Phase 1, so the first gated run is green · **Owner:** @timianmalloo · **Next skill:** `/implement`
