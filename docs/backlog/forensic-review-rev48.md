...
| | |
|---|---|
| **Completed** | **Phases 1, 2 and 3 shipped.** FR-058, FR-059, FR-061, FR-062, FR-063, FR-065, FR-067 RESOLVED; FR-060 resolved as instance + detection. Every control observed **red before green**, FR-062's end-to-end on real CI. CTRL-D registered `controlled`. FR-066's oracle obtained |
| **Remaining** | **Three open:** FR-064 (canonical review filename), FR-066 (document the linear-history rule — oracle now in hand), **FR-068** (the `docs-index.js` timestamp blocking FR-060's source fix). Plus one **recommendation deliberately not imposed**: make `pack-consistency` a required status check on `main` |
| **Best next action** | Decide the required-status-check question — it is the only remaining item that would have prevented the original incident outright, and it is a workflow choice for the owner, not an agent's call |
