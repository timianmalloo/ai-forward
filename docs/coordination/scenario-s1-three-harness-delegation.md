...
## Planned vs actual (2026-09-20, execution 04:14Z-04:45Z)
| track | planned | actual | seam / ladder | channel evidence (quoted in the note or the ledger) | join |
|---|---|---|---|---|---|
| G s1-grok (Grok Build 1.0.34) | 40 calls · 20 min | relaunched by the operator in its tree ≈ 04:25Z; mails acked 04:25:49Z; request acked with the brief's blob (stale=False); note committed `a19c6bd`; decision request 04:33Z; done 04:33Z — ≈ 8 min of track time | 0 kicks | ledger: host-fired `PreToolUse` heartbeat row `"host": "grok"` → **heartbeat enforced**; doorbell `additionalContext` line: not seen; refused Stop while its request was open: not seen → both stay observed-only | Ruling 3 (replace); text applied to `grok-surface.md` §Hooks with one amended sentence (heartbeat now enforced) |
| C s1-codex (codex-cli 0.155.1) | 40 · 20 | no relaunch (thread `01a0bce5…`); pointer queued 04:15:03Z → mails read and acked 04:15:16Z, then the session **stopped** (the pointer said only "run mail read --ack"); kick rung 1 + second pointer 04:34Z → request acked, note `90cc0e9`, decision request and done 04:38Z | 1 kick (rung 1), resolved by the kick | both queued pointers quoted verbatim in the note → **`codex queue` push verified**; no hook channels exist on Codex (unsupported, unchanged) | Ruling 4 (add); Coordination section appended to `codex.md` |
| A s1-agy (Antigravity 1.2.7) | 40 · 20 | relaunched ≈ 04:26Z; mails acked 04:26:22Z; request received 04:27Z; no artifact by the 1500 s deadline (04:39:38Z); `request expire` ran the fallback (first attempt at 04:39:21Z refused: COORD-REQUEST-NOT-DUE, 16 s left) | 1 kick (rung 1, 04:29:51Z, acked); expired-fallback | ledger: session-start and request receive rows, **no heartbeat row** (neither PostToolUse nor Stop) → every Antigravity channel stays observed-only; the Stop contradiction is unresolved by observation | coordinator wrote the fallback note; its proposed §Hooks applied to `agy-surface.md`; no ruling (no request from the track) |
| Coordinator | ≈ 50 calls · 45 min | ≈ 40 calls · 31 min from the first delegate mail to the last ruling; one scripted delegation step (re-run once after a zsh word-splitting bug sent nothing but a premature board notice, corrected by a second notice); two rulings; one expiry; three worktrees cut and registered | leader lapsed twice (D13 TTL), reclaimed at epochs 6-7 | `.agents/harness-status.json` unchanged (dispatch was not part of this test) | linear landing as revision 82 |

**Did it pay?** Two of three tracks returned real artifacts within budget with zero coordinator authoring; the third exercised the ladder and the fallback exactly as written. Three channel facts were established that no headless probe could have produced: Grok's heartbeat fires from a live session, the Codex push reaches the model as a turn (and a pointer that names only the read produces only the read), and Antigravity as launched wrote no heartbeat at all. One boundary defect in the coordinator's own tooling (the mail bodies named worktree paths that `coord worktree new` does not mint) was corrected by a follow-up note to each session.

**Carried forward:** the doorbell pointer text names the action after the read (design-message-layer §8, one sentence); `coord worktree new` should print the path it will mint before creating, or accept `--path`; the Antigravity heartbeat/doorbell probe with `printenv AGENT_SESSION` in the session; the Grok Stop gate probe (a stop with an open request, watched); `coord kick` on a `received` request should carry the deadline (the track showed `deadline not recorded`).

## Status
| | |
|---|---|
| **Completed** | plan; the run (three delegations, two rulings, one expiry with fallback); the join (three surface docs updated from the notes, README rows from quoted evidence, revision 82) |
| **Remaining** | the carried-forward items above; Antigravity and Grok stop-gate observations |
| **Best next action** | in the still-open Antigravity session: `printenv AGENT_SESSION`, one `view_file`, then read `.agents/log/s1-agy.jsonl` — the one probe that separates "hooks did not load" from "no identity in the environment" |
