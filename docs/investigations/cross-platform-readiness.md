...
- **Status:** Root causes verified (six classes); fix plan in `docs/plans/cross-platform-readiness.md` — **P0 and P1 landed at revision 73**, P2–P5 open
...
| XP-07 | `docs-graph.py:339-346,1118-1125` | `tempfile.mkstemp(text=True)` opens with `_O_TEXT`; the CRT translates LF→CRLF beneath Python, so `newline="\n"` on `fdopen` cannot prevent CRLF in `docs/docs-index.js` | Windows | S2 | Verified (line) / Inferred (effect) |
...
| T-3 | `tests/docs_explorer/test_audit_selfcheck.py:134` | `--since HEAD` grandfathering fails on this Mac; cause not established | S3 | Flagged |
