...
| T-3 | `tests/docs_explorer/test_audit_selfcheck.py:134` | `--since HEAD` grandfathering fails on this Mac, on the `macos-latest` runner and on `windows-latest`, and passes on `ubuntu-latest` (CI run 35449895490) — environment-dependent, cause not established | S3 | Flagged |
