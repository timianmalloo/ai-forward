...
- **Obsidian lock-in by accident.** If memory tooling starts *depending* on Obsidian plugins (Dataview queries embedded in canonical docs), the "works without Obsidian" promise breaks. Mitigation: canonical docs stay plain Markdown + the pack's own `docs-graph.py`; Obsidian config is optional; `obsidian-setup.py --init` commits the vault config and git-ignores only per-user state (`.gitignore` lines 42–48). *(Verified — Obsidian downsides research)*
- **Doctor false confidence.** A doctor that only checks file presence can pass while the install is subtly broken (stale graph, wrong revision). Mitigation: compose `docs-graph.py validate|freshness`, not just `Test-Path`. *(Verified — `pack-doctor.py` composes `docs-graph.py` for the graph half, per its header line 11; 2026-09-19)*
