...
| **Remaining** | `/specify` for P0–P2 and P7 against ai-de; live probes of the Copilot and Antigravity doorbells (docs-verified, execution pending — status `observed-only` until a drain is seen); the `prompt-templates/<harness>.md` texts (Inferred until edit-distance measurements exist); `verify-skill-contracts.py` red-first against the 27 skills. Pre-existing gate 3 failures (`master`-branch tests) are unchanged and not introduced here. |
| **Best next action** | `/specify` P7 (compile stage) first — it is the cheapest item after P0, the maintainer's stated priority, and every dispatcher consumes its output; then P2 (leader designation) against ai-de. |
