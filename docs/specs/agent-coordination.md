...
|---|---|---|
| **F1** ~~open~~ **CLOSED 2026-08-24** | Copilot's (and other harnesses') edit-time hook surface may not exist or may differ, leaving those agents advisory-only at the edit boundary. | **Resolved by execution, not by reading docs.** A live Copilot CLI 1.0.80 session honoured a deny: a read of an unleased file succeeded, a write to a leased one was refused with our reason rendered verbatim into the transcript, and the file was unmodified. `HARNESS_STATUS` in `coord-core.py` now records both harnesses as `enforcing`. **Residual, unchanged:** Copilot fails OPEN on a 30s hook timeout, so a hung hook allows; the measured check is 63ms p95 and the commit floor backs it. **Also found by the conformance suite:** the payloads are not the same shape — Copilot's preToolUse arrives as a JSON *array*, its path field is `path` not `file_path`, and the path is absolute, so a hook reading `tool_input.file_path` returns allow for every Copilot edit — a silent no-op wearing the shape of enforcement. |
