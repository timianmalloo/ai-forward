...
revision: 17
released: ''
counts: { lenses: 23, skills: 17, knowledge_docs: 25, templates: 19, scripts: 10 }
...
changes:
  ...
  - { type: changed, area: graph-tooling, paths: ['scripts/docs-graph.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'Graph derivation now emits docs-index/v2 identities, limits, and safe local HTML knowledge surfaces; extracts Mermaid fences heading-locally; exposes deterministic bounded context packets/errors; rejects linked, changed, or reparse-point sources; publishes atomically; serializes append-only JSONL updates through persistent sibling locks; writes Unicode rollups as UTF-8 on Windows; and parses an empty scalar (e.g. a stubbed `review-by:`) as "" rather than [], so a freshly stubbed-then-derived artifact no longer trips a phantom index_drift and /adopt passes its own validate gate.' }
  - { type: added, area: explorer-runtime, paths: ['scripts/docs-explorer-core.js'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'Dependency-free deterministic Explorer state/layout core: normalized typed graph, URL state, selection/context separation, search, stable Graph lanes, context-rooted Mind-map BFS, deterministic Spatial 3D world coordinates, camera targeting, and perspective projection.' }
  ...
  - { type: changed, area: explorer-ui, paths: ['templates/docs-explorer.template.html'], deploy: 'replace docs/index.html from the template, substituting __PROJECT__; preserve docs/docs-index.js', summary: 'Replaces the randomized CDN/React canvas with a polished local Browse-first knowledge portal: responsive Details/Visualization routes, explicit Explore-neighborhood flow, deterministic Graph/Mind-map/Spatial 3D projections, selected-node camera focus with reduced-motion handling, relationship-list parity, history/focus restoration, fail-closed limits, local source/error states, and direct links to audit, documentation, design-preview, and other safe HTML knowledge surfaces.' }
  - { type: changed, area: audit-viewer, paths: ['templates/audit-explorer.template.html', 'scripts/audit-log.py', 'knowledge/audit-and-change-log.md'], deploy: 'copy the script and knowledge doc, then regenerate docs/audit/index.html with audit-log.py render', summary: 'Audit Explorer now uses dependency-free local DOM APIs instead of unpinned CDN runtimes, preserves searchable disclosure/copy/filter/change-log interactions offline, fails explicitly on missing or invalid data, and HTML-escapes the generated project title.' }
  ...
  - { type: added, area: tests, paths: ['../tests/docs_explorer/', '../playwright.config.js', '../package.json', '../package-lock.json'], deploy: 'source-only pack-maintenance/browser regression suite; not deployed', summary: 'Adds deterministic Python and Node contract tests plus Chromium/Firefox/WebKit interaction, recovery, offline, accessibility-route, parity, containment, concurrency, process-bound, benchmark, Spatial 3D camera/orbit/fallback, HTML-surface discovery, and Windows UTF-8 rollup coverage.' }
  ...
  - { type: changed, area: install-skills, paths: ['commands/addpacktorepo/SKILL.md', 'commands/updatepack/SKILL.md'], deploy: 'copy to .claude/skills/<name>/SKILL.md and .github/prompts/<name>.prompt.md', summary: '/addpacktorepo now writes the *.jsonl.lock + spikes/ .gitignore hygiene step as part of its deployment map (previously only documented in INSTALL §2), and /updatepack now applies non-file deploy directives (e.g. the .gitignore addition) instead of silently skipping changelog entries that are neither a copy nor a managed-block re-paste — so a new or updating repo actually receives the lock-file ignore rule.' }
  - { type: added, area: knowledge, paths: ['knowledge/model-orchestration.md'], deploy: 'copy to .claude/knowledge/ and wrap the Copilot instruction (applyTo "**")', summary: 'Model-Orchestration Standard (M1–M12): reflexively applies LOA tier-allocation (P1/P2/1.1/6.1/6.6) to the pack itself — decomposes every skill into 9 activity archetypes and routes each to the cheapest sufficient model tier. The Orchestrator auto-dispatches (human overrules), efficiency-by-default with a cost knob (best model on the highest-rigor work), adversary review runs on a model distinct from the author (hard rule), deterministic mechanics run as cross-platform scripts while the skill surface remains (skills-centric), optimized for the GitHub Copilot CLI on Windows/macOS. Governed by the AI Systems Engineer; the model choice is recorded in the audit log. Knowledge docs 24→25.' }
  - { type: added, area: graph-tooling, paths: ['scripts/model-router.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'Deterministic, stdlib-only, cross-platform routing lookup for the Model-Orchestration Standard: archetype/skill + efficiency|cost profile -> capability tier + illustrative model/effort, enforcing adversary independence (E always frontier + distinct) and never downgrading a hard-veto or high-rigor (define-architecture/investigate) surface. Scripts 9→10.' }
---
