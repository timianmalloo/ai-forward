...
bundle_version: '2026.08.02.3'
revision: 20
released: ''
counts: { lenses: 23, skills: 18, knowledge_docs: 30, templates: 22, scripts: 11 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/code-knowledge-graph.md'], deploy: 'copy to .claude/knowledge/ and wrap as .github/instructions/code-knowledge-graph.instructions.md (applyTo "**")', summary: 'NEW STANDARD — composing Graphify (graphify.com, Apache 2.0, PyPI `graphifyy`, on-device code knowledge graph) with the pack. GK1-GK16. THREE GRAPHS, ONE KNOWLEDGE BASE: the docs graph holds INTENT and is the record; the code graph holds REALITY and is a derived build output; Obsidian is the human lens. GK6-GK7 map Graphify edge tags onto the pack confidence vocabulary — EXTRACTED=Verified, INFERRED=Inferred, AMBIGUOUS=Flagged — which makes end-to-end-integrity E15 (never assert own-code shape from memory) cheap to satisfy with a cited file:line traversal, while keeping a citation from being a promotion. GK4 is DE-DUP NOT BLANKET IGNORE and is repo-kind dependent: in the pack-source repo pack/ is canonical and the four projections are ignored; in a CONSUMING repo there is no pack/, so .claude/ and docs/ai-forward-pack/ are the ONLY copy and must be kept — measured, a blanket ignore there drops 153 files of which 97 exist nowhere else. GK9 maps `affected` to the code-side blast radius feeding the E7 change-surface list; GK10 maps `god-nodes` to the code-side hub watch; GK11 is the join. GK13-GK15 cover on-device egress, the MCP server as a trust boundary, and package disambiguation.' }
  - { type: added, area: scripts, paths: ['scripts/graphify-setup.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'NEW DEPLOYABLE SCRIPT — stdlib only, idempotent, --dry-run everywhere. --install (uv tool install graphifyy + register the /graphify skill for claude/copilot), --init (writes a REPO-KIND-AWARE .graphifyignore: detects pack-source vs consumer vs plain and emits the right de-dup rules, recording the detected kind in the file header), --build (removes graphify-out/ first because incremental update is fail-closed and would keep stale nodes), --join (the novel part: joins the code graph to the docs graph and writes docs/lenses/code-doc-join.md with edge provenance, documentation-with-no-implementation, and risk-with-no-governance), --check (warns and exits 2 when an existing .graphifyignore excludes .claude/ or docs/ai-forward-pack/ in a repo where those are the only copy).' }
  - { type: changed, area: knowledge, paths: ['knowledge/obsidian-lens.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instruction', summary: 'CORRECTION. The revision-19 naming note claimed there is no plugin named Graphify in the Obsidian registry. That is true and was the wrong question: Graphify is a separate product, an on-device code knowledge graph, not an Obsidian plugin. The note now says so and hands off to code-knowledge-graph.md; the Obsidian plugin providing in-vault metrics remains knowledge-graph-analysis.' }
  - { type: changed, area: tools, paths: ['../../tools/setup-knowledge-graphs.ps1'], deploy: 'source-only pack-maintenance tooling; not deployed', summary: 'Renamed from setup-obsidian-for-repo.ps1 and extended to stand up BOTH lenses across several repos in one command: -Graphify adds install + repo-kind-aware ignore + full build + code/doc join; -SkipObsidian runs the code graph alone.' }
  - { type: changed, area: install-hygiene, paths: ['adapters/INSTALL.md'], deploy: 'add graphify-out/ to the consuming repository .gitignore (graphify-setup.py --init does this automatically)', summary: 'The code graph is a build output regenerable from source at any commit; committing it creates a second thing that can be stale.' }
---
