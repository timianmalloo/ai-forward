...
bundle_version: '2026.06.27.1'
revision: 12
released: '2026-06-27'
counts: { lenses: 23, skills: 16, knowledge_docs: 23, templates: 17, scripts: 6 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/audit-and-change-log.md'], deploy: 'copy to .claude/knowledge/ AND wrap as .github/instructions/audit-and-change-log.instructions.md (applyTo all files)', summary: 'New knowledge doc — the Audit & Change Log Standard: a durable, committed, append-only audit log (docs/audit/audit-log.jsonl) of every meaningful prompt/skill/script and a curated change log (docs/audit/change-log.jsonl) of design decisions, so work compounds across sessions instead of evaporating when a session ends. Defines the schema (shortname/datetime/session/prompt/summary + kind/skill/tool/artifacts/tags/outcome/git), the Audit Mandate AL5 (every skill appends as its last action) and Change Mandate CL1–CL2 (collectknowledge/define-architecture/design/migrate capture the decision + git before/after), knowledge-graph integration (the docs/audit/audit-log.md hub node), and the session-history relationship (the committed counterpart to the ephemeral session store). Distinct from the revision-11 prompt-log (a personal prompt-reuse journal in .aiforward/): the audit log is the committed project audit/decision record with a change log, git capture, graph node, and HTML viewer. Knowledge docs 22→23.' }
  - { type: added, area: script, paths: ['scripts/audit-log.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'New stdlib-only script for the audit & change logs: append/change/list/search/get/render/git-context/suggest/import. The JSONL logs are the source of truth; it regenerates the derived window.AUDIT_DATA (audit-data.js) and self-bootstraps the viewer. The only writer/reader of record. Scripts 5→6.' }
  - { type: added, area: template, paths: ['templates/audit-explorer.template.html'], deploy: 'copy to docs/ai-forward-pack/templates/ (the viewer self-bootstraps to docs/audit/index.html on the first append/render)', summary: 'New self-contained interactive viewer over the audit & change log: a searchable (session/date/keyword) timeline, expandable rows, copy-prompt, and a Full-history↔Changes toggle; React UMD + htm with a no-CDN plain-table fallback; loads only audit-data.js so it works over file://. Templates 16→17.' }
  - { type: added, area: skill, paths: ['commands/auditlog/SKILL.md', 'adapters/copilot/prompts/auditlog.prompt.md'], deploy: 'copy SKILL.md to .claude/skills/auditlog/SKILL.md and the prompt to .github/prompts/auditlog.prompt.md', summary: 'New /auditlog CLI skill — the command-line lens over the history: list the last N actions, search by session/date/keyword, recall a past prompt verbatim to re-run, toggle the change timeline, open the viewer, and suggest unlogged changes. A reader/dispatcher over audit-log.py (it logs nothing itself). Skills 15→16.' }
  - { type: changed, area: skill, paths: ['commands/*/SKILL.md', 'adapters/copilot/prompts/*.prompt.md'], deploy: 'RE-COPY the 13 reasoning-workflow skills + prompts to .claude/skills/ and .github/prompts/', summary: 'All 13 reasoning-workflow skills (and their Copilot prompts) now integrate with the audit log: each appends an audit-log entry as its last action (Audit Mandate AL5), and collectknowledge/define-architecture/design/migrate additionally append a change-log entry capturing the decision + git before/after (Change Mandate CL1–CL2).' }
  - { type: changed, area: managed-block, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE both managed blocks wholesale between their markers', summary: 'Skills/Workflows 15→16 (added /auditlog to the lists, alongside the revision-11 /prompts + /searchprompts); new Audit & change log bullet pointing at docs/audit/ and the standard.' }
---
...
| Script bundle (`scripts/*.py` — `docs-graph.py` + `audit-log.py` + `prompt-log.py` + `foundation-check.py` + `pack-doctor.py` + `scrub.py`) | `docs/ai-forward-pack/scripts/` (skills invoke docs-graph for all graph mechanics — V18 — and audit-log for the audit/change logs — AL0.1; prompt-log powers `/prompts` + `/searchprompts`; needs Python 3.8+, stdlib only) | same (shared) |
