...
bundle_version: '2026.06.14.7'
revision: 9
released: '2026-06-14'
counts: { lenses: 23, skills: 13, knowledge_docs: 21, templates: 16, scripts: 4 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/technical-ui-design.md'], deploy: 'copy to .claude/knowledge/ AND wrap as .github/instructions/technical-ui-design.instructions.md (applyTo all files)', summary: 'New base-knowledge doc for the UX lenses on technical/scientific/quantitative interfaces (CAD, scientific viz, computational notebooks, financial/spreadsheet modeling, Monte Carlo simulation, math systems) — TQ1–TQ12: density-with-hierarchy, numerical legibility, perceptually-uniform colormaps (viridis; NEVER rainbow/jet), data-ink, uncertainty-first (fans/tornado/HOPs; error bars are misread), direct-manipulation-plus-precision, units, provenance, reactive recompute, large-data performance. Knowledge docs 20→21.' }
  - { type: changed, area: knowledge, paths: ['knowledge/ui-archetype-catalog.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instruction', summary: 'Added Section G — six technical/scientific/quantitative archetypes: G1 Parametric Modeling Workbench (CAD), G2 Scientific Visualization Pipeline (CFD/FEA), G3 Computational Notebook (Jupyter/Observable; reactive variant covers Desmos/GeoGebra math), G4 Computational Spreadsheet/Grid (financial modeling), G5 Probabilistic/Uncertainty Explorer (Monte Carlo risk/sports/forecasting), G6 Multi-Panel Data Terminal (Bloomberg). Worked-example + provenance sections renumbered to H/I.' }
  - { type: changed, area: knowledge, paths: ['knowledge/ui-archetype-grammar.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instruction', summary: 'Versioned-vocabulary additions (G16) for the Section G archetypes: Arch:SpatialBounded; Nav:Ribbon; Layout:{ViewportWorkbench, VisualizationPipeline, NotebookCells, ComputationalGrid, ScenarioExplorer, MultiPanelWorkstation}.' }
  - { type: changed, area: knowledge, paths: ['knowledge/ui-interaction-design.md'], deploy: 'copy to .claude/knowledge/ and re-wrap the Copilot instruction', summary: 'The "technical and dense domains" paragraph now points to technical-ui-design.md (TQ1–TQ12) and catalog Section G for the expert end of the spectrum.' }
  - { type: changed, area: agent, paths: ['adapters/claude-code/agents/ux-accessibility.md', 'adapters/claude-code/agents/ux-researcher-ia.md'], deploy: 'copy to .claude/agents/ (and .github/agents/ for Copilot, stripping tools:)', summary: 'Both UX lenses now cite technical-ui-design.md + catalog Section G; the UX & Accessibility veto extends to rainbow/jet colormaps, bare point-estimates for stochastic values, non-tabular/unit-less numerics, and missing stale/recompute states on technical surfaces.' }
---
