...
revision: 25
counts: { lenses: 23, skills: 18, knowledge_docs: 33, templates: 22, scripts: 12 }
...
changes:
  - { type: added, area: knowledge, paths: ['knowledge/ui-craft-detection.md'], deploy: 'copy to .claude/knowledge/; wrap with applyTo:"**" frontmatter into .github/instructions/ui-craft-detection.instructions.md', summary: 'CD1-CD19 - the deterministic control beneath the UI craft doctrine, adopting the Impeccable detector (impeccable.style, Apache-2.0) as a rung-2 automated control. The argument is the pack own CI6 control ladder: a lesson recorded as prose is a memoir, and the entire UI craft doctrine sat at rungs 3-4. The proof is dogfooded - run against this pack own mockup-harness template the detector found three real defects it had documented in prose and shipped anyway. THE SEAM IS FREE: the detector resolves DESIGN.md at repo root or docs/, which is exactly the artifact U3a already mandates, so it enforces U3/U20 (tokens, never raw values) OUTWARD against built source, which design-lint.py only ever checked inward. CD5 both linters run; CD6 all 59 rules mapped to owning pack directives so the tool stays subordinate; CD11-CD12 findings translate into the DX22 rubric with accessibility Major-minimum/Blocker-under-obligation and token-discipline Major-minimum floors; CD13-CD15 a clean run is a floor never a verdict and never optimize for the detector; CD16 suppression is a recorded deviation; CD17 the honest adoption ledger (solo maintainer, fast release cadence, pin the version).' }
  - { type: added, area: knowledge, paths: ['knowledge/ui-visual-assets.md'], deploy: 'copy to .claude/knowledge/; wrap with applyTo:"**" frontmatter into .github/instructions/ui-visual-assets.instructions.md', summary: 'VA1-VA18 - the generative visual-asset pipeline (reference backend: Higgsfield MCP) with the guardrails that keep it honest. Closes the last gap in DX4 (anchor to references, not adjectives) by letting the build code toward a concrete image. Hard prohibitions first: VA5 NEVER generate the interface itself (image models render illegible text and invented controls, and a generated screenshot makes a design look finished while encoding nothing); VA4 generate once, download, optimize, COMMIT, reference by relative path - three independent reasons, of which the decisive one is verified: provider retention is 7 days, so a committed CDN URL is a wall of broken images in eight; VA6 mood never structure; VA7 a generated reference is evidence of nothing and never replaces a real named one; VA9 never upload a real likeness or customer data (Privacy hard veto). Plus VA3 the honest preset inventory (the style catalogue skews fashion/social - the product-usable subset is named), VA10 cost as Token-Budget-Throttle, VA12 the asset manifest as a Receipt Ledger, VA13 alt text at generation time, VA14 the perf budget, VA16 character references as consistent harness personas, VA17 motion in the inventory with a reduced-motion fallback.' }
  - { type: added, area: scripts, paths: ['scripts/ui-craft-gate.py'], deploy: 'copy to docs/ai-forward-pack/scripts/', summary: 'Stdlib wrapper performing the CD11 translation: runs the detector, gives every finding the pack DX22 shape (location/dimension/severity/evidence/fix/confidence), applies the CD12 severity floors, emits the DX23 measurement block plus a paste-ready markdown review section (--markdown) or JSON (--json), and gates on Blockers (--gate, --a11y-obligation). Auto-resolves the detector (PATH > local node_modules > npx). Guards the success-shaped failure: a target that does not exist exits 2 rather than passing, because a detector that scanned nothing is not a passing gate (CD9/E14).' }
  - { type: changed, area: skills, paths: ['commands/ui-design/SKILL.md', 'adapters/copilot/prompts/ui-design.prompt.md', 'commands/implement/SKILL.md', 'adapters/copilot/prompts/implement.prompt.md', 'commands/design/SKILL.md', 'adapters/copilot/prompts/design.prompt.md'], deploy: 'copy skills to .claude/skills/<name>/SKILL.md and prompts to .github/prompts/<name>.prompt.md', summary: '/ui-design gains the optional visual world at Stage 1 (after the words, before the design language, with the VA5-VA7 limits), BOTH linters at Stage 3 measure, the translated detector findings at the Stage 4 gate with the CD12 floors and the CD13-CD14 floor-not-a-verdict caveat, the committed asset manifest at Stage 5, and four new definition-of-done items. /implement gains the craft gate as a red-first build-time control with the CD9 non-empty-corpus and own-status checks. /design names the gate in the test plan and plans any generated asset to VA. The AI Systems Engineer and Privacy lenses are convened when assets are generated.' }
  - { type: changed, area: templates, paths: ['templates/mockup-harness.template.html', 'templates/docs-explorer.template.html'], deploy: 'copy to docs/ai-forward-pack/templates/; docs-explorer also regenerates docs/index.html on the next skill run', summary: 'DOGFOOD FIX - the four defects the newly adopted detector found in the pack own artifacts, swept as a class rather than fixed as instances (CI2). mockup-harness: the side-tab accent border (border-left:3px, the single most recognizable tell of AI-generated UI, which DX3 names in prose) replaced by a full hairline over a tinted ground; the transition on max-width removed (animating a layout property causes thrash, U17/DX20, and switching viewport in a review harness is a discrete comparison not a journey). docs-explorer: two more side-tabs plus an inset 3px stripe which is the same tell rotated 90 degrees - the type colour now rides the full hairline, keeping the semantic and dropping the slop; the orphaned --notice-border-width token removed.' }
  - { type: changed, area: knowledge, paths: ['knowledge/continuous-improvement.md'], deploy: 'copy to .claude/knowledge/ and re-wrap into .github/instructions/', summary: 'Seed defect-class register gains three classes found by this work: UX-C craft rule documented but not controlled (status CONTROLLED - it is the class that adopting the detector both revealed and closed), VA-A generated asset referenced by a provider URL, VA-B generated interface passed off as design.' }
  - { type: changed, area: managed-blocks, paths: ['adapters/managed-blocks/CLAUDE.block.md', 'adapters/managed-blocks/AGENTS.block.md'], deploy: 'RE-PASTE wholesale between the AI-FORWARD-PACK:BEGIN/END markers in CLAUDE.md and AGENTS.md', summary: 'Two new bullets after UI craft: the detection control (what it enforces, that it IS the Stage-3 measurement, and that a clean run is a floor never a verdict) and generated visual assets (generate what the UI shows never the UI itself; commit do not link; manifest, alt text, disclosure; no likeness or customer data).' }
  - { type: added, area: evals, paths: ['evals/cases/ui-craft-detection-01.json'], deploy: 'copy to the pack evals directory (not deployed to consuming repos)', summary: 'Red-first and PROVEN red-first: the two anti-pattern assertions (side-tab accent border, layout-property transition) were executed against the pre-fix mockup-harness template from git and BOTH matched - they would have failed - and neither matches post-fix. Also asserts the detector is named in the review, no provider CDN URL survives, alt text is present, and the hard states still ship.' }
---
