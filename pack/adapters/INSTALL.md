...
changes:
  - { type: added, area: templates, paths: ['templates/ui-capability-guide.template.html'], deploy: 'copy to docs/ai-forward-pack/templates/; instantiate verbatim as docs/ui-guide.html, then run docs-graph.py derive so it registers as a guide surface', summary: 'THE HOW-TO LAYER over the seven UI standards. The repo accumulated a lot of interface capability - seven standards, one craft workflow, two linters, four templates, ~30 archetypes, a persona council with vetoes - and none of it answered the question a person actually arrives with: I have a UI job, what do I run, in what order, and what does each thing decide. Eight sections: the layer stack with each standards directive prefix; a job-to-path picker (six situations); the five /ui-design stages and their outputs; a copy-paste command sheet for the workflow, the controls and the graph; a condensed archetype picker across the temporal/operational/spatial/streaming/authoring/technical families; the veto table with who clears what; the generic-look tells mapped to the detector rule that catches each, plus the registered defect classes; and where artifacts land. Self-contained, no build step, no CDN, opens over file://. Derived from the standards and explicitly NOT authoritative over them.' }
  - { type: added, area: docs, paths: ['docs/ui-guide.html', 'docs/ui-guide.md'], deploy: 'repo-local instantiation of the template plus its V2 graph hub node (the .html is data, the .md is the node)', summary: 'DOGFOODED AGAINST ITS OWN CONTROL. First detector run reported 69 findings; every one was fixed in the artifact and none suppressed. Real fixes: kicker-above-heading removed (the craft floor bans eyebrow labels outright - no brief earns them back), the font shorthand converted to longhand so the type scale is actually analysable (which cleared flat-type-hierarchy), three dead tokens deleted, and the hardcoded prefers-contrast hex block replaced with standards-track forced-colors system colours. That last one was the honest fix rather than a suppression: the 68 low-contrast findings were false positives from a static analyser flattening custom properties inside a media query, PROVEN by computing every pairing across all three modes (0 failures), and forced-colors both removes the false positive and is the correct modern mechanism. Final state: clean under --gate --a11y-obligation.' }
  - { type: changed, area: scripts, paths: ['scripts/docs-graph.py'], deploy: 'copy to docs/ai-forward-pack/scripts/; re-run derive', summary: 'Added a guide surface kind so a how-to surface is described as one in the Docs Explorer instead of falling through to the generic knowledge-tool. Matched on guide.html or any *-guide.html, ordered after audit and documentation and before design-preview.' }
  - { type: changed, area: docs, paths: ['docs/DESIGN.md'], deploy: 'repo-local; a consuming repo extends its own ramp if it adds a long-form surface', summary: 'Type ramp gained display-lg 32px and display-sm 18px. A long-form reading surface needs more scale contrast than app chrome does, and the existing ramp (22/14/14/12/12) is deliberately flat for dense UI. Recorded as a design-system addition with a rationale rather than a lint fix, per CD15: laundering the ramp to silence a hierarchy warning is the wrong move.' }
