...
| `claude-code.settings.hooks.json` | Claude Code | merge into `.claude/settings.json` | The `hooks` object for `PreToolUse` (matcher `Read` for the guard; unmatched for the doorbell), `UserPromptSubmit`, `SessionStart` and `SubagentStart`. Committed project settings run in sub-agents too. |
