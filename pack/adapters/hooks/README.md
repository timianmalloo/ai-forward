...
| `agy.ai-forward-hooks.json` | Antigravity (`agy`) | `.agents/hooks.json` | Antigravity hook config: four named sections (`reread-guard` on `PreToolUse`/`view_file`, `session-start` and `mail-doorbell` on `PreInvocation`, `heartbeat` on `PostToolUse`/`Stop`), **each with `"enabled": true`** — the S1 test (2026-09-20, Antigravity 1.2.7) showed the CLI load all five named sections from two files and fire only the one that carried the flag (the operator's user-level hook); ours lacked it and left no row, marker or accumulator. Cause Inferred from the binary's `json:"enabled"` tag and the working example; a live row confirms it. |
...
| Antigravity | none documented | — | unsupported (the script exits 0 for `--host agy`). S1 2026-09-20: the session's `Stop` never reached our hooks because no section was enabled (row above); re-test after the flag |
