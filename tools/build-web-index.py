...
def main():
    ...
    header = "// Generated by tools/build-web-index.py — do not hand-edit. Re-run after pack changes (wired into sync-pack.ps1).\n"
    body = "window.PACK_INDEX = " + json.dumps(payload, ensure_ascii=False, indent=0) + ";\n"
    rendered = header + body

    # FR-060 (class). The portal generator has had a --check drift mode since it was written;
    # this one did not, which is exactly why the portal half of a stale pair was caught by
    # gate 1 while web/pack-index.js was only ever caught by gate 2 - and gate 2 does not run
    # until gate 1 is green. One of a pair being observable is how the other one hides.
    if "--check" in sys.argv:
        try:
            ...
            current = None
        if current != rendered:
            print("DRIFT: web/pack-index.js is stale - re-run tools/build-web-index.py",
                  file=sys.stderr)
            return 1
        print("pack-index.js is current (%d items)." % len(items))
        return 0
